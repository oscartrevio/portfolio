...
export function Step({
	...
	return <div className={cn("grow mb-6", className)}>{children}</div>;
}

export function Steps({ children }: { children: ReactElement[] }) {
	...
	return (
		<section className='flex flex-col h-full w-full grow pb-6'>
			{/* STEPS CONTENT */}
			{children[index]}
			...
			<div className='p-2 flex flex-row w-fit gap-4 mx-auto relative'>
				<div
					className={cn(
						`absolute top-0 left-0 h-full bg-[#20C65D] rounded-full z-10 transition-all duration-300`
					)}
					style={{
						width: `${1.5 + index * 1.5}rem`,
					}}
				></div>
				...
				{children.map((child, i) => {
					return (
						<div
							key={i}
							className={cn("w-2 h-2 rounded-full bg-white z-20", {
								"bg-gray-300": i > index,
							})}
						></div>
					);
...
export function StepBackButton({ children }: { children: ReactNode }) {
	...
	return (
		<button
			className={cn(
				"px-6 py-3 rounded-full bg-gray-100 transition-all duration-300 animate-in animate-accordion-up absolute left-0 font-rounded font-bold",
				{
...
export function StepNextButton({ children }: { children: ReactNode }) {
	...
	return (
		<button
			className={cn(
				"px-6 py-3 rounded-full bg-[#0277FF] font-rounded font-bold text-white w-full transition-all duration-300 flex flex-row gap-1 items-center justify-center",
				{
					"ml-24": !isFirstStep,
				}
			)}
		...
		>
			{isLastStep && <Icons.CircleCheckFilled className='inline w-4 h-4' />}
